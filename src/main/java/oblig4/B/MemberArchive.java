...
public class MemberArchive {
    ...
    public boolean checkMembers(){
        AtomicBoolean ifFail = new AtomicBoolean(false);
            memberHashMap.values().forEach(e -> {
                try {
                    checkMember(e);
                } catch (IllegalImmigrantException ex) {
                    ex.printStackTrace();
                    ifFail.set(true);
                }
            });
    ...
    private void checkMember(BonusMember member) throws IllegalImmigrantException {
        if(member == null){
            throw new IllegalImmigrantException("Get out");
        }
    ...
    public static void main(String[] args) throws IllegalImmigrantException {
