...
class TestBonusMember {
    public static void main(String[] args) throws Exception {
        ...
        if (b1.findQualificationPoints() == 0 && b1.getPoints() == 30000) {
            System.out.println("Test 1 ok");
        }
        b1.registerPoints(15000);
        if (b1.findQualificationPoints() == 0 && b1.getPoints() == 45000) {
            System.out.println("Test 2 ok");
        }
        BasicMember b2 = new BasicMember(110, tove, LocalDate.of(2007, 3, 5));
        b2.registerPoints(30000);
        if (b2.findQualificationPoints() == 30000 && b2.getPoints() == 30000) {
            System.out.println("Test 3 ok");
        }
        SilverMember b3 = new SilverMember(b2.getMEMBER_NO(), b2.getPERSONALS(),
                b2.getENROLLED_DATE(), b2.getPoints());
        b3.registerPoints(50000);
        if (b3.findQualificationPoints() == 90000 && b3.getPoints() == 90000) {
            System.out.println("Test 4 ok");
        }
        GoldMember b4 = new GoldMember(b3.getMEMBER_NO(), b3.getPERSONALS(),
                b3.getENROLLED_DATE(), b3.getPoints());
        b4.registerPoints(30000);
        if (b4.findQualificationPoints() == 135000 && b4.getPoints() == 135000) {
            System.out.println("Test 5 ok");
        }
        testdato = LocalDate.of(2008, 12, 10);
        if (b4.findQualificationPoints() == 0 && b4.getPoints() == 135000) {
